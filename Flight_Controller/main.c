...
//**************************************** MPU6050 setup    ***************************************************
	...
	}while( !(I2C1_readData(0x1B)==0x18) ||	!(I2C1_readData(0x6B)==0x00) );	// Repeat until you read what you wrote.
...
//**************************************** GYROSCOPE offset ***************************************************
	while(i<3000)					// Acquire 3000 samples and sum them all.
	{
		...
		GYRO_X_Offset	+= (int16_t)(0xFFFF & (((GyroscopeP->XAxisH) << 8) | (GyroscopeP->XAxisL)));
		GYRO_Y_Offset	+= (int16_t)(0xFFFF & (((GyroscopeP->YAxisH) << 8) | (GyroscopeP->YAxisL)));
		GYRO_Z_Offset	+= (int16_t)(0xFFFF & (((GyroscopeP->ZAxisH) << 8) | (GyroscopeP->ZAxisL)));
		i++;					// Increment counter.
	}
